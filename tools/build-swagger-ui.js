...
//
// Assumptions:
// - For each version V, you have a folder: openapi-V/
// - The root spec openapi-V/openapi.yaml exists (generated by tools/generate-root-openapi.js V)
...
function listOcpiVersionsFromFolders() {
  // Detect folders like openapi-2.3.0, openapi-2.2.1, ...
  const entries = fs.readdirSync(ROOT_DIR, { withFileTypes: true });
  return entries
    .filter((e) => e.isDirectory() && e.name.startsWith("openapi-"))
    .map((e) => e.name.replace(/^openapi-/, ""))
...
function main() {
  ...
  if (!versions.length) {
    console.error("ERROR: No OCPI versions found. Expected folders like openapi-2.3.0/");
    process.exit(1);
  }
  ...
  for (const version of versions) {
    const srcDir = path.join(ROOT_DIR, `openapi-${version}`);
